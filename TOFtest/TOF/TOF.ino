...
// --- TOF INSTELLINGEN ---
const uint16_t objectDrempelMm = 100;   // stop als object dichterbij dan 10 cm
const uint16_t tofMaxGeldigMm  = 1200;  // metingen > dit = ruis/niks, negeren
const unsigned long tofPrintInterval = 200; // hoe vaak afstand printen (ms)
...
// --- TOF STATUS ---
bool tofOk = false;
unsigned long tofLaatstePrint = 0;
...
void setup() {
  ...
  tof.setTimeout(100);
  if (tof.init()) {
    tof.setMeasurementTimingBudget(20000); // 20ms per meting = snel
    tof.startContinuous();
    tofOk = true;
    Serial.println("TOF VL53L0X OK");
  } else {
    Serial.println("!! TOF init MISLUKT - controleer bedrading SDA=21 SCL=22");
  }

  kalibreerRobot();
  Serial.println("\n=== LIJNVOLGEN + TOF GESTART ===");
}
...
void loop() {
  ...
  // --- TOF CHECK: stop bij object ---
  if (tofOk) {
    uint16_t afstandMm = tof.readRangeContinuousMillimeters();

    // Periodiek afstand printen voor tuning
    unsigned long nu = millis();
    if (nu - tofLaatstePrint >= tofPrintInterval) {
      Serial.print("TOF: ");
      Serial.print(afstandMm);
      Serial.println(" mm");
      tofLaatstePrint = nu;
    }

    // Geldige meting? (VL53L0X geeft soms 8190 bij timeout/geen target)
    bool geldig = (afstandMm > 0 && afstandMm < tofMaxGeldigMm && !tof.timeoutOccurred());

    if (geldig && afstandMm < objectDrempelMm) {
      remmen();
      Serial.print(">> OBJECT! afstand = ");
      Serial.print(afstandMm);
      Serial.println(" mm - GESTOPT");
      return; // skip de rest van deze loop
    }
  }
